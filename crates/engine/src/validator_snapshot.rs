...
mod tests {
    ...
    fn snapshot_replays_uncommitted_state_and_cleans_up() {
        ...
        // Mission roots are canonicalized. On Windows that produces a
        // verbatim path, which Git cannot consume as a patch-file argument.
        ...
        let snap_path = std::fs::canonicalize(snapshot_dir.path())
            .unwrap()
            .join("snapshot under test");
