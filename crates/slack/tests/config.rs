...
//! path, plus the three `KRANZ_SLACK_*` env vars). Mutating those is inherently
...
fn from_config_scenarios() {
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        // --- 4. Env wins over file, per field ---------------------------
        {
            ...
            write_global_config(
                home.path(),
                r#"{ "slack": { "botToken": "xoxb-file", "appToken": "xapp-file", "channel": "C-FILE" } }"#,
            );
            // Env overrides only the channel; tokens still come from the file.
