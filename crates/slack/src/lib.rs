...
//!
//! ```no_run
//! # async fn wire(repo_root: std::path::PathBuf) -> anyhow::Result<()> {
//! let shutdown = async { /* your Ctrl-C / server-shutdown future */ };
//! kranz_slack::serve_slack(&repo_root, shutdown).await?;
//! # Ok(()) }
//! ```
//!
...
pub async fn serve_slack(
    ...
    let inbound = bridge::run_socket(cfg_in, client_in, repo_in, threads_in, in_shutdown);
