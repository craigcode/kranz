...
2. **Final-gate contract-command output is never scrubbed.** `run_shell_command` output is embedded
   verbatim in `Finding.evidence` (orchestrator.rs:2536-2545) and emitted as `ValidationFinding`
   (orchestrator.rs:2567-2571); `emit` (orchestrator.rs:565) does not scrub. A contract command that
   prints `env` or `.env` content lands unredacted in the append-only log, the WS feed, and report.md
   (whose evidence path is truncate-only, orchestrator.rs:3910-3917).
...
`crates/engine/src/scrub.rs` (397 lines, engine-only, deliberately zero external deps — scrub.rs:8-11):
...
(runner.rs:336-339), the orchestrator turn choke point (`pump_turn`, orchestrator.rs:3073), decisions
(orchestrator.rs:579-590), spec re-scrubs. Everything downstream of events.jsonl — report.md, WS,
...
|---|---|---|---|
| Event ingest | `emit` (orchestrator.rs:565) / `EventLog::append` (event_log.rs:251) | per-site scrubs upstream; the boundary itself scrubs nothing | **SCAN — primary gate**; closes Why 2 structurally |
| Transcript writes | runner.rs:117,147 | scrubbed | same boundary, upgraded detectors |
| Human ingress | goal (orchestrator.rs:322-327), steering (orchestrator.rs:1239-1240), ticket creation/scaffold (`Ticket::scaffold`, ticket.rs:163) | unscrubbed, persisted/committed | **SCAN** — a pasted secret is otherwise permanent |
...
- **Merge gate: waiver by committed fingerprint.** A failed scan names fingerprints; waiving means
  ...
  that file lives relative to the `.kranz/.gitignore` template (orchestrator.rs:4345-4360) and the
...
1. **Close the confirmed leaks + centralize the gate.** Scan at `emit`/`EventLog::append`; scrub
   ValidationFinding evidence (orchestrator.rs:2536-2545→2567-2571), report evidence (orchestrator.rs:3910-3917,
