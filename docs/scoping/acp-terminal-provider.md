...
This document specifies the next implementation. It does not enable terminal
support, qualify a provider or record completion of that ticket.
...
The intended interface is below. These names describe the design; they are not
exported Rust types yet. `Scope` is supervisor-bound context, `Handle` is a
validated scoped lookup, and `Create` contains the effective approved action.
...
trait TerminalProvider: Send + Sync {
    ...
    fn create(&self, scope: Scope, action: Create, authority: Self::Authority)
        -> impl Future<Output = Result<Handle>> + Send;
    fn output(&self, handle: Handle)
        -> impl Future<Output = Result<OutputSnapshot>> + Send;
    fn wait_for_exit(&self, handle: Handle)
        -> impl Future<Output = Result<ExitStatus>> + Send;
    fn kill(&self, handle: Handle)
        -> impl Future<Output = Result<StopReceipt>> + Send;
    fn release(&self, handle: Handle)
        -> impl Future<Output = Result<CleanupReceipt>> + Send;
}
```
