...
| `{"type":"item.started","item":{"type":"command_execution","command":...,"status":"in_progress"}}` | shell command about to run under the sandbox (`read-only` for scrutiny) | `ToolUse { tool: "Bash", summary: item.command, raw }` |
| `{"type":"item.completed","item":{"type":"command_execution","command":...,"aggregated_output":...,"exit_code":...,"status":"completed"}}` | same command's result | `ToolResult { tool: Some("Bash"), denied: exit_code == null && status == "failed" (sandbox refusal), summary: aggregated_output, raw }` |
