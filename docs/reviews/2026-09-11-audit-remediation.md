...
The existing standalone audit policy accepts RUSTSEC-2024-0429 until Tauri
moves to a fixed glib series; `cargo audit --deny unsound` passes with that
policy. No new advisory suppressions or unverified major desktop dependency
migration were introduced. Root workspace and npm audits have no reported
vulnerabilities.
...
## Main integration verification (2026-09-12 UTC)

Integrated the remediation with main `599c734`, preserving the Even G2 client.
The only merge conflict was the changelog; both entries are retained. Independent
review confirmed the Even G2 APIs remain compatible. Source review refreshed all
ten knowledge notes and corrected stale gate and Git-hardening descriptions.

The combined tree passed the full Rust workspace gates again: 2,865 tests,
zero failures, seven existing ignored, Clippy, formatting, and build. Dashboard
gates passed with 201 tests and a consistent embedded bundle; Even G2 gates
passed with 22 tests and a packaged build. Strict Rust documentation, standalone
Tauri compilation and its configured audit, root dependency and license checks,
public-content/history audits, packaging tests, and domain lint also passed.
The existing Tauri advisory acceptance and F2 concurrency limitation above remain.
