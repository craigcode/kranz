...
| `POST /api/missions/:id/permission/answer` | body `{"requestId","bindingDigest","allow"}` → `202 {"queued":true}`; exact pending one-call consent, mutation authority required. Unknown/stale/expired/prohibited allows are `409`; extra actor fields rejected. See [live permissions](acp-live-permissions.md). |
...
| `GET /api/missions/outcomes?windowDays=30` | Existing outcomes plus optional additive `outcomeReasons` (mapping v1): current mission state, historical observations and task-class counts. The inclusive activity window selects missions by latest recorded event and retains their full history; reason categories overlap. Other metrics keep their existing scope. `windowDays` is an integer from 0 to 36525, default 30; invalid values return 400. See [recorded outcome reasons](outcome-reasons.md). |
...
| `POST /api/missions/:id/planning/request-plan` | → `200 {"ready":true, "plan":{...}, "planIdentity":"<sha256>", "estimate":{...CostEstimate}}` or `200 {"ready":false, "reply":"<orchestrator prose>"}` (NotReady returns to conversation) |
| `GET /api/missions/:id/pending-plan` | → `200 {"pending":true,"plan":{...},"planIdentity":"<sha256>"}` or `200 {"pending":false}`. The identity binds the exact preview, including its reviewer requirements |
| `POST /api/missions/:id/approve-pending` | body `{"planIdentity":"<sha256>","start":false}` → atomically approves the matching parked plan → `200 {"branch":"kranz/mission-…","started":false}`. Missing/stale identities or no pending plan return `409`; refresh and review the preview before retrying. Optional `"start":true` also starts the approved mission |
